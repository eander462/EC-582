...
sehat_theta_hat_delta = sqrt(varhat_theta_hat) 
```
...
# Normal CI
sapply(1:5, function(i){boot_theta_hat[[i]] + c(-boot_se_theta_hat[[i]], boot_se_theta_hat[[i]])*1.96}) # CI's are read top to bottom. Going left to right are the five samples from 5 different seeds
```
